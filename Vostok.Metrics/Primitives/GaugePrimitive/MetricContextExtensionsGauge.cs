...
namespace Vostok.Metrics.Primitives.GaugePrimitive
{
    ...
    public static partial class MetricContextExtensionsGauge
    {
        ...
        /// <param name="config">Optional config</param>
        /// <inheritdoc cref="IGauge"/>
        public static IMetricGroup<TFor, IGauge> Gauge<TFor>(this IMetricContext context, string name, ITypeTagsConverter<TFor> typeTagsConverter = null, GaugeConfig config = null)
        {
            config = config ?? GaugeConfig.Default;
            return new MetricGroup<TFor, IGauge>(CreateTagsFactory(context, name, config), typeTagsConverter);
        }         
